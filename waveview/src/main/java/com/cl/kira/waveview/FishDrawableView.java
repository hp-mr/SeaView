...
//		canvas.drawCircle(x, y, radius, mPaint);
        makeTrail(new PointF(random.nextInt(100) * mScreenWidth / 100, random.nextInt(100) * mScreenHeight / 100 + fishDrawable.getMinimumHeight()), ivFish, fishDrawable);
        getHandler().postDelayed(runnable, speed);
    }
...
//		x = event.getX();
//		y = event.getY();
//		ObjectAnimator animator = ObjectAnimator.ofFloat(this, "radius", 0f, 1f).setDuration(2000);
//		animator.start();
//		makeTrail(new PointF(x, y),ivFish,fishDrawable);
//		makeTrail(new PointF(x/2, y),ivFish2,fishDrawable2);
//		makeTrail(new PointF(x, y/2),ivFish3,fishDrawable3);
...
//	}
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    private void makeTrail(PointF touch, ImageView ivFish, final FishDrawable fishDrawable) {
        Path path = new Path();
        ...
        PointF controlF = calculatPoint(fishMiddle, 1.6f * fishDrawable.getHEAD_RADIUS(), angle / 2 + delta);
        ...
        speed=random.nextInt(1000)+2000;
